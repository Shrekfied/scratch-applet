...
class SystemPrims extends Primitives {
  ...
  public Object dispatch(int paramInt, Object[] paramArrayOfObject, LContext paramLContext) {
    switch (paramInt) {
      ...
      case 14:
        paramLContext.tyo.println(Logo.prs(paramArrayOfObject[0]));
        return null;
      case 15:
        paramLContext.tyo.print(Logo.prs(paramArrayOfObject[0]));
        return null;
      case 16:
        return null;
    } 
  ...
  void sleepForMSecs(int paramInt, LContext paramLContext) {
    ...
    while (l2 >= 0L && l2 < paramInt) {
      if (paramLContext.timeToStop)
        Logo.error("Stopped!!!", paramLContext); 
      try {
        Thread.sleep(Math.min(paramInt - l2, 10L));
  ...
  Object prim_parleft(LContext paramLContext) {
    if (ipmnext(paramLContext.iline))
      return ipmcall(paramLContext); 
    Object object1 = Logo.eval(paramLContext), object2 = paramLContext.iline.next();
  ...
  boolean ipmnext(MapList paramMapList) {
    try {
      return ((Symbol)paramMapList.peek()).fcn.ipm;
  ...
  Object ipmcall(LContext paramLContext) {
    ...
    while (!finIpm(paramLContext.iline))
      vector.addElement(Logo.evalOneArg(paramLContext.iline, paramLContext)); 
  ...
  boolean finIpm(MapList paramMapList) {
    if (paramMapList.eof())
      return true; 
    Object object = paramMapList.peek();
  ...
  Object prim_class(Object paramObject, LContext paramLContext) {
    try {
      return Class.forName(Logo.prs(paramObject));
    } catch (Exception exception) {
    ...
    if (paramObject instanceof Object[]) {
      ...
      for (byte b = 0; b < arrayOfObject.length; b++) {
        if (arrayOfObject[b] instanceof Object[])
          str = str + "["; 
        str = str + prstring(arrayOfObject[b]);
        if (arrayOfObject[b] instanceof Object[])
          str = str + "]"; 
